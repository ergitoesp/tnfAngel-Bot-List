...
function search() {
    ...
    if (cards) {
        ...
        if (cardsVisible === 0) {
            document.getElementById('searchMore').innerHTML = `No se encontro el bot <a href="/bots/search/?q=${s}">buscar todos los bots</a>?`;
            document.getElementById('searchMore').style.display = "block";
        } else {
            document.getElementById('searchMore').innerHTML = `Puede que quieras <a href="/bots/search/?q=${s}">buscar todos los bots</a>`
            document.getElementById('searchMore').style.display = "block";
        }
