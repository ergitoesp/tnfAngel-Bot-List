...
route.patch("/:id", auth, async (req, res) => {
    ...
    await Bots.updateOne({ botid: req.params.id }, {$set: { long, description, invite, prefix, support, website, github, tags, owners: { primary: bot.owners.primary, additional: check.users } } })
    req.app.get('client').channels.cache.get(server.mod_log_id).send(`<@${req.user.id}> actualizo <@${bot.botid}>`)
    return res.json({success: true, message: "Agrego el bot", url: `/bots/${bot.botid}`})
});
