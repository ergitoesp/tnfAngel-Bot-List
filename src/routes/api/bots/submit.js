...
route.post("/:id", auth, async (req, res) => {
    ...
    if (original && original.state !== "deleted")
        return res.json({ success: false, message: "Tu bot ya esta en tnfAngel bot list.", button: { text: "Edit", url: `/bots/edit/${bot.id}` } });
    ...
    try {
        await req.app.get('client').channels.cache.find(c => c.id === server.mod_log_id).send(`<@${req.user.id}> ${resubmit ? "re" : ""}envio <@${req.params.id}>: <@&${server.role_ids.bot_verifier}>`);
        return res.json({ success: true, message: "Tu bot fue agregado" })
    } catch (e) {
        return res.json({ success: true, message: "Tu bot fue agregado" })
    }
});
