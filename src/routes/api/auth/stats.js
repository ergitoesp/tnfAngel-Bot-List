...
route.post('/:id', async (req, res) => {
    ...
    if (!auth) return res.json({ success: "false", error: "Error al autorizar." });
    let count = req.body.count ? req.body.count : req.body.server_count;

    if (!count) return res.json({ success: "false", error: "Error al encontrar la cuenta." });
    count = parseInt(count);
    if (!count) return res.json({ success: "false", error: "La cuenta no es un numero." });
    ...
    if (bot.servers.length > 0 && bot.servers[bot.servers.length-1] &&  Date.now() - bot.servers[bot.servers.length-1].time < ratelimit * 1000) return res.json({ success: "false", error: "Estas siendo ratelimtado." });
